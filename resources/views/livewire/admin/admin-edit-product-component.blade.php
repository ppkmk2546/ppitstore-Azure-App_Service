            ...
            <div class="col-12 col-lg-12">
               <div class="card">
                ...
                <div class="panel-body">
                    ...
                    @endif
                    <form class="form-horiozontal" wire:submit.prevent="updateProduct">
                        <div align="center">
                            <p style="font-size: 18px; margin-bottom: .65rem;">Preview Product Image</p>
                            @if($newimage)
                                <img src="{{$newimage->temporaryUrl()}}" alt="preview new image" width="250px" style="padding-bottom: 15px" />
                            @else
                                <img src="{{asset('assets/images/products')}}/{{$image}}" alt="current image" width="250" style="padding-bottom: 15px" />
                            @endif
                        </div>
                        <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="input-1">Product Image</label>
                            <input style="cursor: pointer" type="file" class="form-control" wire:model="newimage" />
                        </div>
                        <div class="form-group col-md-6">
                            <label for="input-2">Product Name</label>
                            <input type="text" class="form-control" id="input-2" placeholder="Product Name" wire:model="name" wire:keyup="generateSlug" required />
                            @error('name') <span class="text-danger">{{$message}}</span> @enderror
                        </div>
                        <div class="form-group col-md-12">
                            <label for="input-4">Short Description</label>
                            <textarea class="form-control" id="input-4" placeholder="Short Description" wire:model="short_description"></textarea>
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <input style="cursor: pointer" type="file" class="form-control" wire:model="newimages" multiple />
                        ...
                        <div align="center">
                            @if($newimages)
                                ...
                                @foreach($newimages as $newimage)
                                    @if($newimage)
                                        <img src="{{$newimage->temporaryUrl()}}" width="150px" style="padding-bottom: 15px" />
                                    @endif
                                @endforeach
                            @else
                                @foreach($images as $image)
                                    @if($image)
                                        <img src="{{asset('assets/images/products')}}/{{$image}}" width="150px" style="padding-bottom: 15px" />
                                    @endif
                                @endforeach
                            @endif
                        </div>
